...
export abstract class Constraint {
  ...
  toString(): string {
    ...
    if (this.teamName) str += ' (' + this.teamName + ')';
  ...
  getWarnings(): string[] {
    ...
    if ([ConstraintType.GT, ConstraintType.GTE, ConstraintType.Interval].indexOf(this.getType()) !== -1) {
      if (isNaN(this.minValue)) warnings.push(minNaNWarning);
      else if (this.minValue < 0) warnings.push(minNumberPositiveWarning);
    }

    if ([ConstraintType.LT, ConstraintType.LTE, ConstraintType.Interval].indexOf(this.getType()) !== -1) {
      if (isNaN(this.maxValue)) warnings.push(maxNaNWarning);
      else if (this.maxValue < 0) warnings.push(maxNumberPositiveWarning);
    }
